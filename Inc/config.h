...
#define N_MOT_MAX       800            // [rpm] Максимальное ограничение скорости двигателя(обороты в минуту)
...
*/
  // #define DUAL_INPUTS                     // ADC*(Primary) + PWM(Auxiliary). Uncomment this to use Dual-inputs
    ...
    #define AUX_INPUT2            3, -1000, 0, 1000, 100  // TYPE, MIN, MID, MAX, DEADBAND. See INPUT FORMAT section
  #else
    #define FLASH_WRITE_KEY       0x10052  // Flash memory writing key. Change this key to ignore the input calibrations from the flash memory and use the ones in config.h
